...
> [!proposition]
> The tensor product is a [[Functoriality#^653948|bifunctor]] from the [[Structure of Categories#^2f5c3a|category]] of vector spaces to itself.

> [!definition] Tensor Algebra
> The *tensor algebra* of a [[Vector Spaces#^f4b63e|vector space]] $V$ is the free associative unital algebra generated by $V$. Concretely, it is the direct sum of tensor powers of $V$: $$T(V) = \bigoplus_{n=0}^{\infty} V^{\otimes n},$$where $V^{\otimes 0} = F$ (the underlying [[Ring, Field and Integral Domain#^575174|field]]), and the multiplication is given by the tensor product: $$(v_1 \otimes \cdots \otimes v_m) \cdot (w_1 \otimes \cdots \otimes w_n) = v_1 \otimes \cdots \otimes v_m \otimes w_1 \otimes \cdots \otimes w_n.$$ ^b6f3e2
> 
...
> [!proposition]
> Suppose $V$ and $W$ are finite dimensional $F$-vector spaces. Then the canonical linear map 
> $$ W^{*} \otimes V^{*} \to (V \otimes W)^{*},\qquad \psi\otimes\phi \mapsto (x\otimes y \mapsto \phi(x)\psi(y)) $$
> is an isomorphism.
> 
