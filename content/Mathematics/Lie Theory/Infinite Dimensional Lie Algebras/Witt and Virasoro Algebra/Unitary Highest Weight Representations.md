...
> for all $v\in\H^{\infty}$ and $w\in \H^{-\infty}$, where $\H$ is the [[Complete Metric Space#^concept-08babb55cabd|completion]] of $V$ with respect to $\left< \cdot,\cdot \right>$, and $\{\newcommand{\H}{\mathcal{H}}\H^{t}\}_{t\in\R}$ are [[Nelson's Commutator Theorem#^eec324|scales]] of $\H$ associated with $A:=\overline{\pi(L_{0})}$.
