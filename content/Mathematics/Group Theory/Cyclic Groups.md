...
> that is, every $g \in G$ has the form $g = a^n$ for some $n \in \Z$. We say that $G$ is generated by $a$. ^c52ddd
...
> [!proposition]
> Let $G = ⟨a⟩$ be a cyclic group. Then exactly one of the following holds:
> 1. $G$ is infinite
> 2. There exists $n\in\N$ such that $G =\{e,a,a^2,...,a^{n−1}\}$.
> $\quad$

*Proof*  If $G$ is finite, To prove exists $n\in\N$ such that $G =\{e,a,a^2,...,a^{n−1}\}$ is equivalent to prove that for all $k\in \Z$, exists $n<k$ and $r<n$ such that $a^k=a^r$. By the pigeonhole principle, for any integer $l$ exists $m$ that $m<l$ we have $a^l=a^m$. Hence $a^{l-m}=e$, so $a$ has a finite order $o(a)$. Let $n=o(a)$. Then for every $k \in \Z$, $k=qn+r$ with $q\in\Z$ and $0\leq r<n$. It follows that $$ a^k=a^{n q+r}=\left(a^n\right)^q a^r=e^q a^r=a^r. $$$\square$

![[Cosets and Lagrange’s Theorem#^coro]]

![[Cosets and Lagrange’s Theorem#^c1062e]]
