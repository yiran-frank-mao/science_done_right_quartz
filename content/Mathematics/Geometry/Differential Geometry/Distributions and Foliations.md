...
> [!corollary] 
> Suppose $M$ is a smooth manifold, $\mathcal{D}$ is an involutive $k$-plane distribution on $M$, and $S \subseteq M$ is a codimension-$k$ [[Manifolds, Atlases and Smooth Structures#^632f7d|embedded submanifold]]. If $p \in S$ is a point such that $T_p S$ is complementary to $\mathcal{D}_p$, then there is a flat chart $(U, (s^i))$ for $\mathcal{D}$ centered at $p$ in which $S \cap U$ is the slice $s^1 = \cdots = s^k = 0$.
