...
> Any global flow $\Phi_t$ on a [[Manifolds, Atlases and Smooth Structures#^6ef2ef|smooth manifold]] $M$ induces a cotangent lift flow $\tilde{\Phi}_t$ on the [[Tensor Fields#^f98eed|cotangent bundle]] $T^*M$. For any $(q, \alpha) \in T^*M$ (where $q \in M$ and $\alpha \in T_q^*M$), this flow is defined by $$\tilde{\Phi}_t(q,\alpha) = (\Phi_t(q), \Phi_t^*\alpha),$$ where the [[Tensor Fields#^bd6c33|covector]] $\Phi_t^*\alpha \in T_{\Phi_t(q)}^*M$ is determined by the condition $$\Phi_t^*\alpha(v) = \alpha(\d_{\Phi_t(q)}\Phi_{-t}(v)) \quad \text{for all } v \in T_{\Phi_t(q)}M.$$ ^32c822
