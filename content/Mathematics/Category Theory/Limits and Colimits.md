...
>[!definition] Cone
>A *cone* over a diagram $D \colon \mathbf{J} \to \mathbf{C}$ with summit or apex $C ∈ \obj \mathbf{C}$ is a [[Naturality#^d73db0|natural transformation]]  $λ\colon \mathcal{C} \Rightarrow D$ whose domain $\mathcal{C}\colon \mathbf{J}\to\mathbf{C}$ is the constant functor at $C$. The components $λ_j \colon C \to D_{j}$ are called the legs of the cone. Explicitly:
