...
- In a fixed poset category $\mathsf{P}$, every morphism $p ≤ q$ is both monic and epic. Because for any $f\colon p\to q\in \mor\mathsf{P}$, we have unique $g\colon r\to p$ and $h\colon q\to s$. Thus monic and epic.
$\quad$
...
> [!definition] Skeleton ^2e268d
> A *skeleton* of a category $\mathsf{C}$ is a full subcategory $\mathsf{D}$ of $\mathsf{C}$ such that the inclusion functor $I\colon \mathsf{D}\to \mathsf{C}$ is an equivalence of categories and there is at most one object in $\mathsf{D}$ from each isomorphism class of objects in $\mathsf{C}$.
> 
...
<u><b>e.g.</b></u> A [[Groups, Order and Subgroups#^6e0960|group]] $G$ is a groupoid with one object $G$. Clearly that singleton $G$ is a category $\mathsf{G}$. For all arrows $f_{x}\colon G\to G$, we have $f_{x}\circ f_{x^{-1}}= 1_{G}$.

**Lemma**  Any category $\mathsf{C}$ contains a maximal groupoid, the subcategory containing all of the objects and only those morphisms that are isomorphisms.
...
>Let $\mathsf{C}$, $\mathsf{D}$ be categories with binary products. A functor $F \colon \mathsf{C} \to \mathsf{D}$ is said to preserve binary products if
...
<u><b>e.g.</b></u>  The forgetful functor $U \colon\mathsf{Mon} \to \mathsf{Set}$ preserves binary products.

**Corollary**  For any object $X$ in a category $\mathsf{C}$ with products, the (covariant) representable functor $\mathrm{Hom}(X, \cdot) \colon C \to \mathsf{Set}$ preserves products.
**Proof**  For any $A,B ∈ \mathsf{C}$, the [[Objects and Elements#^69c772|proposition]] says that there is a canonical isomorphism $\pi_{X}$:$$\mathrm{Hom}(X,A\times B)\cong\mathrm{Hom}(X,A)\times\mathrm{Hom}(X,B)$$

**Def**  <i><u>Discrete Category</u></i>
A discrete category is a category $\mathsf{C}$ whose only morphisms are the identity morphisms: $$\Hom(X,X)=\{1_{X}\},\quad \Hom(X,Y)=\emptyset$$for all $X\neq Y$ be $\mathsf{C}$-objects.
