...
Let $i\colon S^{n-1} \hookrightarrow D^n$ be the inclusion map. The composition $r \circ i\colon S^{n-1} \to S^{n-1}$ is the identity map, so the composition $$\tilde{H}_{n-1}(S^{n-1}) \xrightarrow{i_*} \tilde{H}_{n-1}(D^n) \xrightarrow{r_*} \tilde{H}_{n-1}(S^{n-1})$$is the identity. However, $\tilde{H}_{n-1}(S^{n-1})\cong \Z$, $\tilde{H}_{n-1}(D^{n})\cong 0$ and $\tilde{H}_{n-1}(S^{n-1})\cong \Z$, it is impossible to compose to the identity, yielding a contradiction. $\square$