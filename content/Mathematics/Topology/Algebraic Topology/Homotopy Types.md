...
> A *deformation retraction* of $X$ onto a subspace $A$ is a homotopy from the identity map of $X$ to a retraction of $X$ onto $A$. That is, a map $r\colon X\to X$ such that $r(X)=A$ and $\newcommand{\id}{\mathrm{id}} r|_{A}=\id$. ^05414e
> 

<u><b>e.g.</b></u>
- A line segment $[0,1]$ deformation retracts to any line segment $[a,b]\subset [0,1]$.
- The unit disk $D^{2}$ deformation retracts to its centre point.
$\quad$
...
> [!proposition]
> If $f\colon X\to Y$ where $Y$ is contractible, then $f$ is null homotopic. That is, it is homotopic to the constant map.
> 

*Proof*  Since $Y$ is contractible, we have maps $g\colon \{*\}\to Y$ and $h\colon Y \to \{*\}$ such that $g\circ h \simeq \id_{Y}$ through some $\alpha\colon Y\times I \to Y$. Then the composite $$X\times I \xrightarrow{f\times\id} Y\times I \xrightarrow{\alpha} Y$$gives the desired homotopy. $\square$
