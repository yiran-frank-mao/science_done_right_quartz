...
>[!definition] Limit on Metric Spaces
...
>[!theorem] 
> Let $(X,d)$ and $(Y,ρ)$ be metric spaces and let $f \colon X →Y$ be a function. The following are equivalent:
...
>[!theorem] 
> Let $(X,d)$ and $(Y,ρ)$ be metric spaces and let $f \colon X →Y$ be continuous. If $K ⊂ X$ is compact, then $f (K )$ is compact.
...
>[!theorem] 
> Continuous function on compact space must be uniformly continuous.
...
<b><u>e.g.</u></b>  Let $(X,d)$ be a metric space. Suppose $A\subset X$ is non-empty. Then we can define the distance of $x$ from $A$ by setting $d(x,A)=\inf_{z\in A}d(x,z)$. Then the function $x \mapsto d(x,A)$ is Lipschitz with Lipschitz constant $1$. Indeed, for all $x,y\in X$ we have $$d(x,A)\leq d(x,z)\leq d(x,y)+d(y,z)\leq d(x,y)+d(y,A)$$ Hence $d(x,A)-d(y,A)\leq d(x,y)$. Similarly, $d(y,A)-d(x,A)\leq d(x,y)$. Thus $$d_{\R}(d(x,A),d(y,A))=|d(x,A)-d(y,A)|\leq d(x,y)$$$\square$
...
> [!theorem]
> Let $d_1$ and $d_2$ be two metrics on $X$ that are Lipschitz equivalent on $X$. Then $d_1$ and $d_2$ are topologically equivalent.

**Corollary**  The metrics induced by equivalent norms are topologically equivalent.

**Lemma**  If $X$ is a [[Vector Spaces#^f4b63e|vector space]] and two norms $\|\cdot \|_1$ and $\|\cdot \|_2$ on $X$ induce topologically equivalent metrics then the norms are equivalent.
...
>[!theorem] 
> Let $(X,d)$ and $(Y,ρ)$ be metric spaces. If $f \colon X →Y$ is continuous and $K ⊂X$ is compact, then $f$ is uniformly continuous on $K$.