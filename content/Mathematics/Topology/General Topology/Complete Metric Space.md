...
Let $(X,d)$ be a [[Metric Spaces#^concept-e26011bc6f0a|metric space]]. A sequence $(x_{n})$ in $X$ is called a *Cauchy sequence* if for any $ε > 0$ there exists $N$ such that $d(x_{n},x_{m}) < ε$ for all $n,m > N$. ^179eeb
...
>[!proposition]
> Let $(X,d)$ be a [[Metric Spaces#^concept-e26011bc6f0a|metric space]] and let $(x_{n})$ be a [[Complete Metric Space#^179eeb|Cauchy sequence]] in $X$. If $(x_{n})$ has a convergent subsequence, then $(x_{n})$ is convergent. ^80b2f4
...
## Quotient of Complete Metric Spaces

> [!proposition]
> Suppose $(X,d)$ is a complete metric space and $A\subset X$ is a compact subspace. Then the [[Constructions on Topological Spaces#^d57887|topological quotient]] $X/A$ is metrizable, and complete under the metric $$d_{X/A}([x],[y])=\min\{d(x,y), d(x,A)+d(y,A)\}.$$
> 

*Proof*  We first show that $d_{X/A}$ is a well-defined metric on $X/A$. For $x,x'\in A$ and $y,y'\in A$, we have $d_{X/A}([x],[y])=\min\{d(x,y), d(x,A)+d(y,A)\}=\min\{0, 0\}=0$ and $d_{X/A}([x'],[y'])=\min\{0,0\}=0$. Therefore, $d_{X/A}([x],[y])=d_{X/A}([x'],[y'])$, which shows that the metric is well-defined. The other properties of a metric (non-negativity, symmetry, and triangle inequality) can be verified similarly. 
Now we show that the topology induced by this metric coincides with the quotient topology by showing that $q(B_{r}(x_{0}))=B_{r}(q(x_{0}))$ for all $x_{0}\in X$. 