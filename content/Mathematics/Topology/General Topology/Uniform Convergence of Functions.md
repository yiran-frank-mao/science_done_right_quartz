...
> [!theorem]
> **Thrm**  Let $f_{n} \colon [a, b] → \R$ be a sequence of continuous functions such that $f_{n} → f$ uniformly on $[a, b]$ for some function $f \colon [a, b] → \R$. Then $$\int_{a}^{b}f_{n}(x)\dd x\to\int_{a}^{b}f(x)\dd x$$  ^cd69c1
