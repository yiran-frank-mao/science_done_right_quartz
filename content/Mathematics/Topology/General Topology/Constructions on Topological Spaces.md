...
> [!lemma]
> The product topology is generated by the [[Topological Spaces#^02668a|subbasis]] $$\mathcal{S}=\{\pi_{i}^{-1}(U_{i})\mid U_{i}\in \mathcal{T}_{i},i\in I\},$$ where $\pi_{j}\colon \prod_{i\in I}X_{i}\to X_{j}$ is the projection map. In particular, it is the coarsest topology on $\prod_{i\in I}X_{i}$ such that all projection maps $\pi_{j}\colon \prod_{i\in I}X_{i}\to X_{j}$ are continuous.
