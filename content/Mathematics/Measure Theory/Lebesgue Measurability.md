...
> A set $E\subseteq \R^{d}$ is said to be *Lebesgue measurable* if for all $\epsilon>0$, there is an open $O\supset E$, such that $$m_{*}(O\setminus E)\leq \epsilon.$$If $E$ is Lebesgue measurable, we write $m(E)$ for the exterior measure of $E$.
> The Lebesgue $\sigma$-algebra on $\R^{d}$ is the [[Measurable Spaces and Functions#^60a516|$\sigma$-algebra]] generated by all Lebesgue measurable sets. ^b5a209
