...
> In particular, the image $f(X)$ is called the *range* of $f$. 
> Given a subset $B ⊂ Y$, its *inverse image* or *preimage* under $f$ is defined by$$f^{-1}(B)=\{x\in X:f(x)\in B\}.$$ ^8f2f00
