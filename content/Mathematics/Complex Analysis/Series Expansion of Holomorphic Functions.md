...
3. $S$ is closed: Observe that $$S=\bigcap_{n=0}^{\infty}\left(f^{(n)}\right)^{-1}(\{0\})$$Since $f^{(n)}$ is continuous, [[Continuous Maps on Topological Spaces#^30fe04|the preimage of a closed set is closed]], so $S$ is closed.
