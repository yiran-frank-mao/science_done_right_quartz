...
> [!lemma]
> Let $D$ be a domain and let $\mathcal{F}⊂H(D)$. Assume that for every compact $K ⊂D$ there exists an $M >0$ such that $|f(z)|≤M$ for all $f ∈\mathcal{F}$ and $z ∈K$. Let $$\mathcal{F}_K=\{f|_K: f\in\mathcal{F}\}\subset C(K)$$Then every sequence in $\mathcal{F}_{K}$ has a subsequence that [[Uniform Convergence of Functions#^a59a2d|converges uniformly]] on $K$.

*Proof*  We will prove by [[Uniform Convergence of Functions#^5cb1ea|Arzelà-Ascoli theorem]], showing that
...
> [!corollary]
> Every simply connected domain $D\subseteq \hat{\C}$ is conformally equivalent to one and only one of the $\Delta$, $\C$ and $\hat{\C}$.
