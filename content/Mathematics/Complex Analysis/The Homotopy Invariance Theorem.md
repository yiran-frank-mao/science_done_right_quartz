...
Since $S$ is the image of a compact set under a continuous map, $S$ is also compact (see [[Continuity on Metric Spaces#^3a05b1|theorem]]). Applying the previous [[The Homotopy Invariance Theorem#^220c21|lemma]], there exists $\varepsilon>0$, such that for all $z\in S$, $\Delta(z,\varepsilon)\subset D$. Since $[0,1]\times[0,1]$ is compact, by [[Continuity on Metric Spaces#^d7d908|theorem]], $\Gamma$ is uniformly continuous, so there is $\delta>0$ such that when $|s-s'|<\delta$ and $|t-t'|<\delta$ we have $|\Gamma(s,t)-\Gamma(s',t')|<\varepsilon$.
