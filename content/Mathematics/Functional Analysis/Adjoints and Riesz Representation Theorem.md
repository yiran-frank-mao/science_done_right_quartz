...
> Let $\ell$ be a continuous linear functional on a Hilbert space $\H$. Then, there exists a unique $g\in\H$, such that $\ell(f)=\langle f, g\rangle$ for all $f\in \H$. Moreover, $\|\ell\|=\|g\|$.
...
> [!proposition] 
> Let $T\colon \H_{1}\to\H_{2}$ be a bounded linear map on separable Hilbert spaces. Then there exists a unique adjoint bounded linear map $T^{*}\colon \H_{2}\to\H_{1}$ such that $$\langle Tf,g\rangle=\langle f,T^{*}g\rangle$$ for all $f\in\H_{1}$ and $g\in\H_{2}$. 
> 
