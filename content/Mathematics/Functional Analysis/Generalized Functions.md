...
> Suppose $M$ is a [[Manifolds and Atlases#^6ef2ef|smooth manifold]] and $E\to M$ is a finite-rank [[Vector Bundles#^9cb683|vector bundle]]. Let $\Gamma_{c}^{\infty}(M, E^{*})$ be the space of compactly supported smooth sections of the dual bundle, then the space of *distributional sections* of $E$ is defined as the continuous dual $$\mathcal{D}'(M,E):= \Gamma_{c}^{\infty}(M,E^{*})'.$$
