The harmonic oscillator model is so important in physics. Consider the Taylor series expansion of an arbitrary potential $V(x)$ with a minimum at $x=x_{0}$:$$\newcommand{\d}{\mathrm{d}}V(x)=V(x_{0})+\frac{\d V}{\d x}\Bigg|_{x=x_{0}}(x-x_{0})+\frac{1}{2}\frac{\d^{2}V}{\d x^{2}}\Bigg|_{x=x_{0}}(x-x_{0})^{2}+\cdots $$We can ignore higher order terms for small amplitude oscillations. We can also subtract the constant $V (x_{0})$ from the potential without changing the force experienced by the particle, leaving $$V(x)=\frac{1}{2}\frac{\d^{2}V}{\d x^{2}}\Bigg|_{x=x_{0}}(x-x_{0})^{2}\equiv\frac{1}{2}qx^{2}$$This is why harmonic oscillator is so important!
