...
5 and 6 are completely similar, $$\begin{aligned}\exp\left(\frac12 \hat{a}^*(M)\hat{a}^*\right)\hat{a}(u)\exp\left(-\frac12 \hat{a}^*(M)\hat{a}^* \right)&=\hat{a}(u),\\\exp\left(\frac12 \hat{a}^*(M)\hat{a}^*\right)\hat{a}^{*}(u)\exp\left(-\frac12 \hat{a}^*(M)\hat{a}^* \right)&=\hat{a}^{*}(u)-\hat{a}(Mu).\end{aligned}$$For the second quantization $\Gamma(L)$, we have $$\Gamma(L)\hat{a}^{*}(u)\Gamma(L)^{-1}(f)=$$