...
> [!proposition] 
> There is a well-defined map $\Gamma\colon \overline{\Ann} \hookrightarrow \CSP(\H)$, by sending an annulus $\Sigma=(S,\Phi^{-},\Phi^{+})$ to an operator $\Gamma_{\Sigma}$ on $\H$ such that it satisfies $$ h\bigg|_{\partial_{-}\Sigma} \circ \Phi^{-}\bigg|_{S^{1}}= f, \quad h\bigg|_{\partial_{+}\Sigma}\circ \Phi^{+}\bigg|_{S^{1}}= \Gamma_{\Sigma}(f) $$for some holomorphic $h\in H(\Sigma)$.

*Proof*  %% TODO: Show that the above equations are solvable  %%
We shall now check that $\Gamma_{\Sigma}$ always lands in $\CSP(\H)$. Let us pick the standard representative $(\Sigma_{t}, \gamma_{+}, \gamma_{-})$, and consider the annulus $[(\Sigma_{t}, e^{-t}, 1)]$
Observe that for $f,g\in\H$, $$g=\Gamma_{\Sigma}(f) \iff V_{(e^{t}\gamma_{+})^{-1}}g = \Gamma_{\Sigma_{0}} (V_{\gamma_{-}^{-1}}f)$$
Note that for $f,g\in\H$, $g=\Gamma_{\Sigma}(f)$ means there exists $h\in H(\Sigma)$ such that $$h\bigg|_{\partial_{-}\Sigma}\circ \gamma_{-}=f,\quad h\bigg|_{\partial_{+}\Sigma} \circ \gamma_{+}=g.$$These are exactly $$